...
//
// Four steps: upload, map, review, commit.
//
// The mapping step is the one that was missing. The upload used to post the
// file and start processing it in the same request, so the first thing an admin
// saw was a finished run against a column mapping nobody had looked at — and
// when the mapping was wrong, a review screen reporting nothing at all with
// nothing to explain why. Now the upload only reads the file's shape; what each
// column means, what the file yields, and how it should be reconciled are all
// decided on a screen that shows the answer before anything is staged.
//
// Nothing writes to the catalogue until the confirm button on step four.
