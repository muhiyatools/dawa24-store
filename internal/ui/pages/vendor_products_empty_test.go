...
func TestVendorProducts_EmptyStateAddFromCatalogButton(t *testing.T) {
	...
	// 2. Must contain the button with data-modal-open and onclick handler
	...
	if !strings.Contains(html, `onclick="openAddFromCatalogModal()"`) {
		t.Errorf("expected empty state button to have onclick='openAddFromCatalogModal()'")
	}
	...
	if !strings.Contains(html, `<button type="button" data-modal-open="add-from-catalog-modal" onclick="openAddFromCatalogModal()"`) {
