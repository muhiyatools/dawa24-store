...
func (h *UIHandler) buildPrintableInvoiceData(ctx context.Context, invoice *billing.Invoice, order *commerce.Order, lang any) (*billing.PrintableInvoiceData, error) {
	...
	if order != nil {
		...
		if issueDate.IsZero() { issueDate = order.CreatedAt }
		if subtotal.IsZero() { subtotal = order.Subtotal }
		if totalDiscount.IsZero() { totalDiscount = order.TotalDiscount }
		if totalAmount.IsZero() { totalAmount = order.TotalAmount }
		paymentMethod = string(order.PaymentMethod)
	...
	if order != nil {
		for _, s := range order.Shipments {
			if s != nil {
				if deliveryCode == "" && s.DeliveryCode != "" { deliveryCode = s.DeliveryCode }
				if trackingNumber == "" && s.TrackingNumber != "" { trackingNumber = s.TrackingNumber }
			}
		}
	}
