...
// WebMCP Integration (Web Model Context Protocol for in-browser AI Agents)
(function() {
  function initWebMCP() {
    var tools = [
      {
        name: "searchCatalog",
        description: "Search pharmaceutical products and medicines in Egypt by brand name, generic name, or barcode.",
        inputSchema: {
          type: "object",
          properties: {
        ...
        execute: async function(params) {
          try {
            var res = await fetch('/compare/search?q=' + encodeURIComponent(params.query || ''), {
              headers: { 'Accept': 'application/json' }
            });
            if (!res.ok) return { error: "Failed to search catalog: " + res.status };
      ...
      {
        name: "compareDiscounts",
        description: "Compare real-time pharmaceutical discounts and commercial conditions across Egyptian drug distributors.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      {
        name: "getCompanyInfo",
        description: "Returns platform overview, customer support channels, and licensing terms for Dawa24.",
        inputSchema: { type: "object", properties: {} },
        execute: async function() {
          return {
            platform: "Dawa24",
            description: "Unified B2B Marketplace for Egyptian Pharmacies & Suppliers",
    ...
    var ctx = window.modelContext || (navigator && navigator.modelContext);
    if (ctx && typeof ctx.provideContext === 'function') {
      ...
      if (!window.modelContext) {
        window.modelContext = {
          tools: tools,
          provideContext: function(c) { window.modelContext.active = c; }
        };
      }
