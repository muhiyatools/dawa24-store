...
func loadAdminCatalogB(e *engine) {
	...
	addKey(e, "admin.finance.invalid_withdrawal_id", "billing", "معرف طلب السحب غير صالح.", "Invalid withdrawal request ID.", "Validation error")
	addKey(e, "admin.finance.withdrawal_approved_success_format", "billing", "تم اعتماد طلب السحب وخصم مبلغ %s ج.م من رصيد المحفظة (معاملة #TX-%d).", "Withdrawal request approved successfully and %s EGP debited from user wallet (Tx #TX-%d).", "Success notice")
	addKey(e, "admin.finance.withdrawal_rejected_success", "billing", "تم رفض طلب السحب وإشعار صاحب الحساب بالسبب بنجاح.", "Withdrawal rejected and user notified with reason.", "Success notice")
	addKey(e, "admin.finance.default_withdrawal_rejection_reason", "billing", "تم رفض طلب السحب لعدم مطابقة بيانات الحساب أو تعذر إتمام التحويل.", "Withdrawal rejected due to invalid account details or transfer failure.", "Default rejection reason")
	...
	addKey(e, "admin.org_changes.title", "admin", "طلبات تعديل بيانات المنشآت", "Organization Profile Change Requests", "Page title")
	addKey(e, "admin.org_changes.subtitle", "admin", "مراجعة وتدقيق واعتماد طلبات تحديث بيانات الهوية والحدود المالية والتراخيص للمنشآت التجارية", "Review, audit and approve profile, financial limit and license change requests from organizations", "Page subtitle")
	addKey(e, "admin.org_changes.tab_all", "admin", "كافة الطلبات", "All Requests", "Tab all")
	addKey(e, "admin.org_changes.tab_pending", "admin", "قيد المراجعة", "Pending Review", "Tab pending")
	addKey(e, "admin.org_changes.tab_approved", "admin", "المعتمدة", "Approved", "Tab approved")
	addKey(e, "admin.org_changes.tab_rejected", "admin", "المرفوضة", "Rejected", "Tab rejected")
	addKey(e, "admin.org_changes.empty_title", "admin", "لا توجد طلبات تعديل مطابقة", "No matching change requests", "Empty title")
	addKey(e, "admin.org_changes.empty_message", "admin", "تمت مراجعة ومعالجة كافة طلبات تعديل بيانات المنشآت بالكامل.", "All organization change requests have been reviewed and processed.", "Empty message")
	addKey(e, "admin.org_changes.diff_modal_title", "admin", "فحص ومقارنة التعديلات المطلوبة للمنشأة", "Inspect & Compare Requested Changes", "Modal title")
	addKey(e, "admin.org_changes.approved_success", "admin", "تم اعتماد طلب التعديل بنجاح وتحديث بيانات المنشأة فوراً.", "Change request approved successfully and organization profile updated.", "Success notice")
	addKey(e, "admin.org_changes.rejected_success", "admin", "تم رفض طلب التعديل وتوثيق سبب الرفض بنجاح.", "Change request rejected and reason recorded successfully.", "Success notice")
	addKey(e, "admin.org_changes.invalid_id", "admin", "معرف الطلب غير صالح.", "Invalid change request ID.", "Error notice")
	addKey(e, "admin.org_changes.not_found", "admin", "طلب التعديل غير موجود أو تم حذفه.", "Change request not found or deleted.", "Error notice")
	addKey(e, "admin.org_changes.rejection_reason_required", "admin", "يرجى كتابة سبب رفض الطلب لتوضيحه للمنشأة.", "Please enter a rejection reason to inform the organization.", "Validation error")
	addKey(e, "admin.org_changes.search_placeholder", "admin", "البحث باسم المنشأة...", "Search by organization name...", "Search placeholder")
	addKey(e, "admin.org_changes.all_sections", "admin", "كافة الأقسام", "All sections", "Filter option")
	addKey(e, "admin.org_changes.date_from", "admin", "من تاريخ", "From date", "Filter label")
	addKey(e, "admin.org_changes.date_to", "admin", "إلى تاريخ", "To date", "Filter label")
	addKey(e, "admin.org_changes.filter_btn", "admin", "تصفية", "Filter", "Button text")
	addKey(e, "admin.org_changes.reset_btn", "admin", "إعادة ضبط", "Reset", "Button text")
	addKey(e, "admin.org_changes.requester", "admin", "مقدم الطلب", "Requested by", "Field label")
	addKey(e, "admin.org_changes.reviewer", "admin", "المراجع", "Reviewed by", "Field label")
	addKey(e, "admin.org_changes.diff_col_field", "admin", "الحقل", "Field", "Table column header")
	addKey(e, "admin.org_changes.diff_col_previous", "admin", "القيمة السابقة", "Previous Value", "Table column header")
	addKey(e, "admin.org_changes.diff_col_proposed", "admin", "القيمة المقترحة", "Proposed Value", "Table column header")
	addKey(e, "admin.org_changes.admin_notes_label", "admin", "ملاحظات الإدارة / سبب الرفض", "Admin notes / rejection reason", "Label")
	addKey(e, "admin.org_changes.view_org", "admin", "عرض المنشأة", "View organization", "Tooltip")
	addKey(e, "admin.org_changes.view_user", "admin", "عرض ملف المستخدم", "View user profile", "Tooltip")
	addKey(e, "vendor.org.request_submitted_success", "vendor", "تم إرسال طلب تعديل بيانات المنشأة بنجاح وهو الآن قيد المراجعة والاعتماد من قبل إدارة المنصة.", "Organization profile change request submitted successfully and is under review by platform admins.", "Success notice")
	addKey(e, "vendor.org.pending_banner_title", "vendor", "يوجد طلب تعديل قيد المراجعة حالياً", "A profile change request is currently pending review", "Alert banner title")
